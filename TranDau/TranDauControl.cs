...
public class TranDauControl : ManualSingleton<TranDauControl>
{
    ...
    public static bool WaitingForServer = true;
    ...
    private IEnumerator CoWaitForServerThenReveal()
    {
    ...
    {
        Debug.Log($"[TranDauControl] Init() called, count={playersData?.Count ?? -1}, waiting={WaitingForServer}");

        if (playersData == null || playersData.Count == 0)
        {
            Debug.LogWarning("[TranDauControl] Init() SKIP: data null or empty");
            return;
        }
        if (playerMove == null || UserData.Instance == null)
        {
            Debug.LogError($"[TranDauControl] Init() SKIP: playerMove={playerMove != null}, UserData={UserData.Instance != null}");
            return;
        }
        ...
        if (WaitingForServer)
        {
            ...
            Debug.Log("[TranDauControl] First snapshot received!");
        }
